...
<template>
  <div class="demo-header">Blockchain-based privacy protection mechanism for electronic health records</div>
  <main class="main">
    <MainCard title="医院端">
      <template v-slot:top>
        <div class="sub-head">
      ...
      <template v-slot:bottom>
        ...
        <div class="sub-content">
          <NButton :disabled="!address || progress >= 100" @click="upload" type="primary" style="width: 100%">
            <div v-if="progress === 0"> Upload to Blockchain</div>
            <div v-else>Upload <span style="padding-left: 20px" v-if="progress">{{ progress + '%' }}</span></div>
          </NButton>
        </div>
        <div v-if="contractAddress" class="contract-address-content">
          <NDivider></NDivider>
          <div style="font-size: 16px; text-align: center; padding-bottom: 10px;">Smart Contract Address: {{ contractAddress }}</div>
        </div>
      </template>
    </MainCard>
  </main>
